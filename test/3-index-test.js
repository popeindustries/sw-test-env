...
describe('sw-test-env', () => {
  ...
  describe('register()', () => {
    it('should execute script path in ServiceWorker context', async () => {
      ...
      expect(sw.__serviceWorker__.self.caches).to.exist;
    });
    it('should define "location" object in ServiceWorker context', async () => {
      await sw.register('sw-empty.js');
      expect(sw.__serviceWorker__.self.location).to.have.property('href', 'http://localhost:3333/sw-empty.js');
    });
  ...
  describe('trigger()', () => {
    it('should trigger an install event', async () => {
      await sw.register('sw-empty.js');
      expect(sw.__serviceWorker__.state).to.equal('installing');
      await sw.trigger('install');
      expect(sw.__serviceWorker__.state).to.equal('installed');
      expect(sw.controller).to.equal(null);
    });
    it('should trigger an install event and emit `updatefound` and `statechange` events', (done) => {
      sw.register('sw-empty.js').then((registration) => {
        registration.addEventListener('updatefound', () => {
          registration.installing?.addEventListener('statechange', (event) => {
            expect(event.target).to.equal(sw.__serviceWorker__);
            expect(registration.waiting).to.equal(sw.__serviceWorker__);
            done();
          });
        });
        sw.trigger('install');
    ...
    it('should trigger a ServiceWorker event handler', async () => {
      ...
      await sw.trigger('install');
      expect(sw.__serviceWorker__.self.installed).to.equal(true);
    });
    ...
    it('should trigger a handled error event', async () => {
      ...
      expect(sw.__serviceWorker__.self.message).to.equal('ooops!');
  ...
  describe('ready', () => {
    it('should execute install/activate lifecyle', async () => {
      ...
      await sw.ready;
      expect(sw.__serviceWorker__.state).to.equal('activated');
      expect(sw.__serviceWorker__.self.installed).to.equal(true);
      expect(sw.__serviceWorker__.self.activated).to.equal(true);
    });
    ...
    it('should execute install/activate lifecyle for multiple connected pages', async () => {
      ...
      await sw.ready;
      expect(sw.__serviceWorker__.state).to.equal('activated');
      expect(sw2.__serviceWorker__.state).to.equal('activated');
      expect(sw.__serviceWorker__.self.installed).to.equal(true);
      expect(sw2.__serviceWorker__.self.activated).to.equal(true);
      expect(await sw.__serviceWorker__.self.clients.matchAll()).to.have.length(2);
    });
    it('should install and cache assets', async () => {
      ...
      expect(sw.__serviceWorker__.state).to.equal('activated');
      // @ts-ignore
      const urls = (await (await sw.__serviceWorker__.self.caches.open('v1')).keys()).map((req) => req.url);
