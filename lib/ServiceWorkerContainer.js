...
module.exports = class ServiceWorkerContainer {
   ...
   * Proxy to scope's implementation
   * @param {String} event
   * @param {Function} fn
   */
  addEventListener (event, fn) {
    if (!this.scope) throw Error('no script registered yet');
    this.scope.addEventListener(event, fn);
  }
  ...
  _load (scriptURL, fetch) {
    ...
    const globalScope = new ServiceWorkerGlobalScope(this._registration, fetch);
